...
RenderPass::RenderPass(not_null<Device*> device, not_null<PipelineFactory*> factory, RenderInfo info)
	: m_info(std::move(info)), m_framebuffer(info.out_framebuffer), m_device(device), m_factory(factory) {
	...
	if (*m_framebuffer) { m_device->destroy(*m_framebuffer); }
	*m_framebuffer = m_device->makeFramebuffer(m_info.renderPass, views, graphics::cast(extent), 1U);
